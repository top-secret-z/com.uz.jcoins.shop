...
class JCoinsShopItem extends DatabaseObject implements IRouteController {
	...
	public function isBuyer() {
		...
		return (isset(self::$buyerCache[$this->shopItemID]) ? self::$buyerCache[$this->shopItemID] : 0);
	}
	...
	public function canSee() {
		if (WCF::getSession()->getPermission('admin.shopJCoins.canManage')) return true;
		
		// check viewable
		$itemList = new ViewableJCoinsShopItemList();
		$itemList->getConditionBuilder()->add('shopItemID = ?', [$this->shopItemID]);
		$itemList->readObjects();
		if (!count($itemList->getObjects())) return false;
		
		if (WCF::getSession()->getPermission('user.jcoins.canShop')) return true;
		if (WCF::getSession()->getPermission('user.jcoins.canSeeShop')) return true;
	...
	public function canBuy() {
		...
		if ($this->typeDes == 'textItem') {
			if ($this->soldOutText()) return false;
		}
		
		if (!WCF::getSession()->getPermission('user.jcoins.canShop')) return false;
		
		if (!JCOINS_ALLOW_NEGATIVE) {
			if (WCF::getUser()->jCoinsAmount < $this->getPrice()) return false;
		}
		
		if ($this->buyLimit && $this->isBuyer() >= $this->buyLimit) return false;
		if ($this->productLimit && $this->sold >= $this->productLimit) return false;
		
		if ($this->expirationStatus && $this->expirationDate < TIME_NOW) return false;
	...
	public function canDownload() {
		if ($this->isDisabled) return false;
		if ($this->typeDes != 'download') return false;
		if (!WCF::getSession()->getPermission('user.jcoins.canShop')) return false;
	...
	public function getPrice() {
		if ($this->isOffer && $this->offerEnd > TIME_NOW) return $this->offerPrice;
		return $this->price;
	}
	...
	public function isOffer() {
		if ($this->isOffer && $this->offerEnd > TIME_NOW) return true;
		return false;
	}
	...
	public function isAllowed() {
		...
		if (in_array($this->shopItemID, $ids)) return true;
